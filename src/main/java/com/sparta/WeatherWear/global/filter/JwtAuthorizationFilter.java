...
public class JwtAuthorizationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain filterChain) throws ServletException, IOException {
        ...
        log.info("Access Token retrieved from request: {}", accessToken);

        if (accessToken != null) { // accessToken 확인
            String accessTokenValue = jwtUtil.substringToken(accessToken); // accessToken 검증
            log.info("Access Token after removing prefix: {}", accessTokenValue);
            ...
            if (jwtUtil.validateToken(accessTokenValue)) {
                ...
                if (newAccessToken != null) {
                    ...
                    log.info("New Authentication set for user: {}", userDetails.getUsername());
                } else {
                    // 리프레시 토큰이 만료된 경우
                    log.error("Refresh Token expired or invalid, cannot generate new Access Token");
